...
def test_TS_RDCC_64_TC_RDCC_65_Event_id_data_completeness(db_connection: connection | None,validation: dict[str, str]):
    ...
    try:
        query = f"""Select dre.event_id from 
        {validation['target_schema']}.{validation['target_table']} dre 
        except 
        SELECT 
        e.event_id 
        FROM (
    ...
    try:
        ...
        if diff_count == 0:
            message = f"✅ Souce-to-Target check for event id passed: All records from {validation['source_table']} exist in {validation['target_table']}."
            logging.info(message)

        else:
            message = f"❌ Souce-to-Target check for event id failed: {diff_count} records in {validation['source_table']} missing from {validation['target_table']}."
            logging.error(message)
...
def test_TS_RDCC_64_TC_RDCC_66_Event_name_data_completeness(db_connection: connection | None,validation: dict[str, str]):
    ...
    try:
        query = f"""SELECT 
                ...
                select dre.event_id,dre.event_name 
                from {validation["target_schema"]}.{validation["target_table"]} dre
                """
        cursor.execute(query)
        differences = cursor.fetchall()
    ...
    try:
        query = f"""select dre.event_id,dre.event_name from 
                    {validation["target_schema"]}.{validation["target_table"]} dre 
                    except
                    SELECT 
                        e.event_id,e.event_name 
...
def test_TS_RDCC_64_TC_RDCC_67_Event_lifecycle_status_Validation(db_connection: connection | None,validation: dict[str, str]):
    print("\nThis Test case validates the event_lifecycle_status in dim_regcor_event is correctly fetched from event & objectlifecyclestate_ref table having 'event_lifecycle__C' records\n")
    cursor = db_connection.cursor()
    ...
    try:
        ...
        query = f"""SELECT 
                    ...
                    select event_id,event_lifecycle_status
    ...
    try:
        ...
        query = f"""select event_id,event_lifecycle_status
                from  {validation["target_schema"]}.{validation["target_table"]}
                except
                SELECT 
                    src.event_id, 
